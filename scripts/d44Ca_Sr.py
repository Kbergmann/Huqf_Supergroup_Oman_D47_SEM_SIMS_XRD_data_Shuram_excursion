...
expected_formations = ['Khufai', 'TopKhufai', 'Birba', 'BuahBirba', 'Shuram', 'Cement', 'CementBurial']
...
for member in df['Formation']:
    if member == 'Khufai':
        ...
        color_list.append('#E0E0E1')  # Dodger Blue  #4B0082
...
def calcium_sr_plot_continuous(ax, df_, colorbar, norm, color_col, alpha):
    for _, row in df_.iterrows():
        Ca44_40 = row['Ca44_40']
        Sr_Ca = row['Sr_Ca_mmol_mol']

        ax.errorbar(x=Ca44_40, y=Sr_Ca,
             yerr=None,
             xerr=None,
             color=row.color, marker=row.marker,
             markersize=6,
...
def make_plot(df, plot_suffix):
    ...
    x_ticks = [-1.4, - 1.2, -1.0, -0.8, -0.6, -0.4, -0.2, 0.0]
...
#    ax.grid(True, color='grey', linewidth=0.3)
    ...
    calcium_sr_plot_continuous(ax, df, 0, 0, df['color'], df['alpha'])

    # Adjust spacings if necessary
    fig.tight_layout()  # Adjust layout to make sure everything fits without overlap

    # Save and close figure
    filename = f'figures/Finals_July2024/{plot_suffix}_square_33_nsc_ncrop_plot_d44Ca_Sr_oman_noKDEs.svg'
    filename2 = f'figures/Finals_July2024/{plot_suffix}_square_33_nsc_ncrop_plot_d44Ca_Sr_oman_noKDEs.pdf'
    plt.savefig(filename, format='svg', bbox_inches="tight", transparent=False, pad_inches=0)  # DPI is not needed for PDF output
